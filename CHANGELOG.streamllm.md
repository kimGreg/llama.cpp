# StreamLLM fork — upstream-file changelog

Chronological list of edits this fork makes to files that are part of
upstream llama.cpp / ggml. Everything under `streamllm-ext/` is fork-
...
ask whether the same goal can be achieved in `streamllm-ext/`.

## 2025 — initial hook wiring

`streamllm-ext: mul_mat hook infrastructure + llama.cpp glue` (`e84856a81`)
...
- `src/llama.cpp`: after a successful `llama_model_load`, call
  `streamllm_ext::install_for_gguf(path_model)` guarded by
  `#if defined(STREAMLLM_EXT_ENABLED)`. Balanced by
  `streamllm_ext::clear()` in `llama_model::~model` (see
  `src/llama-model.cpp`).
...
`streamllm-ext: hook debug infra + graph-capture guard` (`f14a2d322`)
...
- `ggml/src/ggml-cuda/ggml-cuda.cu`: when the hook is installed,
  ...
  default-on but lifted by `STREAMLLM_ENABLE_CUDA_GRAPHS=1`, which
...
`STREAMLLM_ENABLE_CUDA_GRAPHS=1` initially only worked for eager. In
...
session, and (b) the lookahead prefetch (`STREAMLLM_PREFETCH_L`)
issued H2Ds for next-forward tensors with no corresponding wait in
this capture window.

Fix:
- `streamllm-ext/vram_pool.{h,cu}`: `load()` and `launch_copy_()`
...
- `streamllm-ext/runtime.{h,cpp}`: `move_chunk()` grows an optional
  `compute_stream` and plumbs it into `pool.load()`.
- `streamllm-ext/runtime_hook.cpp`: passes `stream` through to
...
+19.7% vs baseline. Safe to default `STREAMLLM_ENABLE_CUDA_GRAPHS=1`
across all scheduler modes; still env-var-gated for the safety
margin.

`streamllm-ext: split-layout GGUF (F16 placeholder + byte blob)` (`86b0d1c8b`)

- `src/llama-model-loader.cpp`: skip tensors whose name starts with
  `streamllm.bytes.` in both GGUF-context readers. These are raw
  byte blobs consumed out-of-band by `StreamllmRuntime`; they have
...
- `src/llama-model-loader.h`: add `std::unordered_set<std::string>
  ...
  `streamllm.managed_tensors` key) + `<unordered_set>` include.
- `src/llama-model-loader.cpp`:
  - Populate `streamllm_managed` in the loader constructor right
    after metadata is loaded. No-op on stock GGUFs.
  - In `load_all_data`, skip the disk copy for managed tensors —
    their bytes are consumed out-of-band by `StreamllmRuntime`.
- `src/llama-model.cpp`:
  ...
  - Add `streamllm_seed_ptrs` to `llama_model::impl` and free the
    seeds in the destructor.
  - Include `<cuda_runtime.h>` inside the `STREAMLLM_EXT_ENABLED`
    guard.
...
Env vars:
- `STREAMLLM_INIT_BPW`         → `STREAMLLM_CHUNKS`
- `STREAMLLM_PREFIX_BPW`       → `STREAMLLM_PINNED_CHUNKS`
- `STREAMLLM_EVICT_TAIL`       → `STREAMLLM_EVICT_UNPINNED`
- `STREAMLLM_PREFETCH_L`       → `STREAMLLM_LOOKAHEAD`
- `STREAMLLM_{FRONT,MID,REAR}_BPW`    → `..._CHUNKS`
- `STREAMLLM_{FRONT,MID,REAR}_PREFIX` → `..._PINNED`
...
- **Encoder (`streamllm/scripts/encode_qwen3.py:57-74`)**: currently
  ...
  (c) emit the expert tensors into `streamllm.managed_tensors`.
- **CUDA hook (`ggml-cuda.cu:2532+`)**: the streamllm hook only fires
...
- `ggml/src/ggml-cuda/ggml-cuda.cu`:
  ...
  - In `ggml_cuda_should_fuse_mul_mat` (ffn_up + ffn_gate + glu),
    ...
    those fused kernels read src0 directly, bypassing the streamllm
