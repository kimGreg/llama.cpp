// DPMoE / core — ModelExecutor ABC.
//
// Model-specific forward executor selected at llama_model_load by the
// GGUF's ``dp_moe.executor`` key.  Owns the forward path for the
...
// the per-layer ``"dp_moe.moe_layer_<L>"`` sentinel emitted by
...
public:
    ...
    // Registry name.  Must match the GGUF's ``dp_moe.executor``
    ...
    // ``reader`` is the parsed dp_moe.* metadata; ``gguf_path``
    // is the file the managed tensors live in (for SSD-stream).
    virtual void bind_to_model(DPMoERuntime &  rt,
    ...
    // dp_moe_pre_op before CUDA dispatch; return false so the stock op
...
// (the GGUF's ``dp_moe.executor`` key, or a default for legacy
