// DPMoE — runtime implementation. Thin facade over
...
namespace dp_moe_ext {
// Provided by runtime_glue.cpp; called when DP_MOE_PROFILE=1 is set
...
// scheduler to make room — measures pool-pressure under DP_MOE_PROFILE=1.
...
// dp_moe_stat_host_dram_bytes() for /dp_moe/stats.
std::atomic<uint64_t> g_host_dram_bytes{0};
}  // namespace dp_moe_ext

extern "C" unsigned long long dp_moe_stat_host_dram_bytes(void) {
    return (unsigned long long)
        dp_moe_ext::g_host_dram_bytes.load(std::memory_order_relaxed);
}
...
// bucket on destruction. Compiles to a no-op when DP_MOE_DIAG is
...
bool host_chunk_cache_enabled() {
    if (const char * s = std::getenv("DP_MOE_HOST_CACHE")) {
        return !(s[0] == '0');
    }
    if (const char * s = std::getenv("DP_MOE_DISABLE_HOST_CACHE")) {
        return !(s[0] && s[0] != '0');
    }
    return true;
}
...
DPMoERuntime::DPMoERuntime(size_t capacity_bytes, int device,
                                   bool copy_stream,
...
void DPMoERuntime::install(const StreamReader & reader,
                               const std::string & gguf_path) {
    if (installed_) {
        throw std::runtime_error(
            "DPMoERuntime: already installed; each instance handles one GGUF");
    }
    ...
    if (gguf_fd_ < 0) {
        throw std::runtime_error(
            "DPMoERuntime::install: cannot open " + gguf_path +
            ": " + std::strerror(errno));
    }
#ifdef O_DIRECT
    if (const char * env = std::getenv("DP_MOE_SSD_O_DIRECT")) {
        if (std::atoi(env) != 0) {
            ...
            if (gguf_direct_fd_ < 0) {
                std::fprintf(
                    stderr,
                    "DPMoE: O_DIRECT open failed for %s: %s; "
    ...
    // than n_used_per_tok so the ring rarely waits on a previous H2D.
    {
        ...
        if (slot_bytes > 0) {
            ...
            if (cerr != cudaSuccess) {
                throw std::runtime_error(
                    std::string("DPMoERuntime::install: cudaHostAlloc(host_ring) failed: ") +
    ...
    // cudaMallocs at install; cold launch goes from 4-5 min to ~30 s.
    {
        ...
        if (slab_bytes > 0) {
            ...
            if (cerr != cudaSuccess) {
                throw std::runtime_error(
                    std::string("DPMoERuntime::install: cudaMalloc(small_slab) failed: ") +
                    cudaGetErrorString(cerr));
            }
    ...
    // DP_MOE_IO_WORKERS=N (0 disables and falls back to
    // inline pread on the hook thread).
    {
        int n_workers = 8;
        if (const char * w = getenv("DP_MOE_IO_WORKERS")) {
        ...
        for (int i = 0; i < n_workers; ++i) {
            io_workers_.emplace_back(
                &DPMoERuntime::io_worker_loop_, this);
        }
    }

    {
        int n_workers = 4;
        if (const char * w = getenv("DP_MOE_BATCH_PREAD_THREADS")) {
            n_workers = std::atoi(w);
            if (n_workers < 0) n_workers = 0;
            if (n_workers > 32) n_workers = 32;
        }
        ...
        for (int i = 0; i < n_workers; ++i) {
            batch_pread_workers_.emplace_back(
                &DPMoERuntime::batch_pread_worker_loop_, this);
        }
    }
...
int DPMoERuntime::streaming_pread_fd_(const void * dst,
                                          int64_t off,
                                          size_t nbytes) const {
#ifdef O_DIRECT
    if (gguf_direct_fd_ >= 0) {
...
void DPMoERuntime::pread_span_(uint8_t * dst, int64_t off,
                                   size_t nbytes) const {
    ...
    while (total < nbytes) {
        ...
        if (got < 0) {
            int err = errno;
            throw std::runtime_error(
                "DPMoERuntime::pread_span_: pread failed: " +
                std::string(std::strerror(err)));
        }
        if (got == 0) {
            throw std::runtime_error(
                "DPMoERuntime::pread_span_: short pread");
        }
        total += (size_t)got;
    }
    if (fd == gguf_fd_) {
...
bool DPMoERuntime::run_batch_preads_(
...
void DPMoERuntime::register_layout(const std::string & wid,
    ...
    void ** d_qbias  = nullptr;
    {
        ...
        if (small_slab_ == nullptr ||
            ...
            throw std::runtime_error(
                "DPMoERuntime::register_layout: small_slab exhausted");
        }
...
void DPMoERuntime::release_host_bytes(const std::string & wid) {
...
void DPMoERuntime::release_chunk_host(const std::string & wid, int cid) {
    auto it = entries_.find(wid);
    if (it == entries_.end()) return;
...
bool DPMoERuntime::host_resident(const std::string & wid, int cid) const {
    auto it = entries_.find(wid);
...
void DPMoERuntime::register_chunk_io(
...
void DPMoERuntime::register_chunk_io(
    ...
    if (it == entries_.end()) {
        throw std::runtime_error(
            "DPMoERuntime::register_chunk_io: unknown wid " + wid);
    }
    if (chunk_file_offsets.size() != chunk_file_sizes.size() ||
        ...
        throw std::runtime_error(
            "DPMoERuntime::register_chunk_io: source vector size mismatch for " +
            wid);
    }
...
const UpstreamLayoutDevice * DPMoERuntime::layout(const std::string & wid) const {
    auto it = entries_.find(wid);
    if (it == entries_.end()) return nullptr;
    return &it->second.dev;
}

ChunkedTensor * DPMoERuntime::tensor(const std::string & wid) const {
    auto it = entries_.find(wid);
    if (it == entries_.end()) return nullptr;
...
DPMoERuntime::tensor_anybcq(const std::string & wid) const {
    auto it = entries_.find(wid);
    if (it == entries_.end()) return nullptr;
    return dynamic_cast<anybcq::AnyBCQFamilyTensor *>(it->second.tensor.get());
}

void DPMoERuntime::clear_chunk_device_ptr(const std::string & wid, int cid,
                                              StreamHandle stream) {
...
void DPMoERuntime::begin_deferred_pointer_clears_(StreamHandle stream) {
    defer_pointer_clears_ = true;
    deferred_clear_stream_ = stream;
    deferred_clear_patches_.clear();
}

void DPMoERuntime::flush_deferred_pointer_clears_() {
...
void DPMoERuntime::cancel_deferred_pointer_clears_() {
    defer_pointer_clears_ = false;
    deferred_clear_stream_ = nullptr;
    deferred_clear_patches_.clear();
}

void * DPMoERuntime::small_alloc(size_t bytes) {
...
bool DPMoERuntime::prepare_chunk_payload_(
    const std::string & wid, int cid, PreparedChunkPayload & out)
{
    auto it = entries_.find(wid);
    if (it == entries_.end()) {
        throw std::runtime_error(
            "DPMoERuntime::prepare_chunk_payload_: unknown wid " + wid);
    }
    Entry & e = it->second;
    ...
    if (!cid_is_chunk(cid)) {
        throw std::runtime_error(
            "DPMoERuntime::prepare_chunk_payload_: unknown cid " +
            std::to_string(cid));
    }
    ...
    int64_t total = 0;
    {
        ...
        while (total < size) {
            ...
            if (got < 0) {
                int err = errno;
                throw std::runtime_error(
                    "DPMoERuntime::prepare_chunk_payload_: pread failed: " +
                    std::string(std::strerror(err)));
            }
            if (got == 0) {
                throw std::runtime_error(
                    "DPMoERuntime::prepare_chunk_payload_: short pread for " + wid);
            }
            total += got;
        }
    }
    ...
    if (host.any_precision) {
        if (p < 0 || p >= (int)host.chunk_planes.size()) {
            throw std::runtime_error(
                "DPMoERuntime::prepare_chunk_payload_: any-prec chunk index "
    ...
    if ((size_t)size != expected_source_bytes) {
        throw std::runtime_error(
            "DPMoERuntime::prepare_chunk_payload_: SSD-stream plane size "
    ...
    if (kernel_ready_source) {
        ...
        throw std::runtime_error(
            "DPMoERuntime::prepare_chunk_payload_: missing disk_to_kernel_fn for " +
            wid);
    } else {
...
void DPMoERuntime::prepare_chunk_payloads_batch_(
        ...
        std::vector<uint8_t> span((size_t)span_size);
        {
            ...
            while (total < span_size) {
                ...
                if (got < 0) {
                    int err = errno;
                    throw std::runtime_error(
                        "DPMoERuntime::prepare_chunk_payloads_batch_: pread failed: " +
                        std::string(std::strerror(err)));
                }
                if (got == 0) {
                    throw std::runtime_error(
                        "DPMoERuntime::prepare_chunk_payloads_batch_: short pread");
                }
                total += got;
            }
        ...
        for (size_t j = i; j < last; ++j) {
            ...
            if (host.any_precision) {
                if (pb.p < 0 || pb.p >= (int)host.chunk_planes.size()) {
                    throw std::runtime_error(
                        "DPMoERuntime::prepare_chunk_payloads_batch_: "
...
void DPMoERuntime::finish_loaded_chunk_(
...
void DPMoERuntime::finish_loaded_chunks_batch_(
...
EventHandle DPMoERuntime::move_chunks_batch(
    const std::vector<ChunkKey> & keys,
    StreamHandle compute_stream)
{
    ...
    auto ensure_batch_staging = [&](size_t nbytes) {
        ...
        if (cerr != cudaSuccess) {
            throw std::runtime_error(
                std::string("DPMoERuntime::move_chunks_batch: "
    ...
    auto try_move_kernel_ready_bundle_batch = [&]() -> bool {
        ...
        for (const auto & key : keys) {
            ...
            if (host.any_precision) {
                if (p < 0 || p >= (int)host.chunk_planes.size()) {
                    throw std::runtime_error(
                        "DPMoERuntime::move_chunks_batch: any-prec chunk "
            ...
            if (src_size < 0 || (size_t)src_size != kernel_chunk_bytes) {
                throw std::runtime_error(
                    "DPMoERuntime::move_chunks_batch: bundle kernel-ready "
        ...
        auto try_decode_pread_ahead = [&]() -> bool {
            ...
            if (const char * env =
                    std::getenv("DP_MOE_BATCH_PREAD_THREADS")) {
            ...
            if (const char * env =
                    std::getenv("DP_MOE_BATCH_PREAD_AHEAD_MAX_MB")) {
                const long mb = std::strtol(env, nullptr, 10);
            ...
            if (const char * env =
                    std::getenv("DP_MOE_BATCH_PREAD_COALESCE_GAP_KB")) {
...
EventHandle DPMoERuntime::move_chunk(const std::string & wid, int cid,
                                          Tier src, Tier dst,
                                          StreamHandle compute_stream) {
    DP_MOE_NVTX_RANGE("move_chunk");
    ...
    if (it == entries_.end()) {
        throw std::runtime_error(
            "DPMoERuntime::move_chunk: unknown wid " + wid);
    }
    Entry & e = it->second;
    UpstreamLayoutHost & host = e.tensor->host();
    UpstreamLayoutDevice & dev = e.dev;
    ...
    if (cid == kCidQBias) {
        ...
        if (src_ptr == nullptr && have_io && host_ring_ != nullptr) {
            ...
            if ((size_t)size > host_ring_slot_bytes_) {
                throw std::runtime_error(
                    "DPMoERuntime::move_chunk: chunk size (" +
            ...
            const bool prof = (getenv("DP_MOE_PROFILE") != nullptr);
            const auto t_pread_start = prof
                ? std::chrono::steady_clock::now()
                : std::chrono::steady_clock::time_point{};
            DP_MOE_NVTX_RANGE("pread");
            uint64_t pread_ns = 0;
            {
                DiagSpan _t(diag::MoveSite::Pread);
                ...
                while (total < size) {
                    ...
                    if (got < 0) {
                        int err = errno;
                        throw std::runtime_error(
                            "DPMoERuntime::move_chunk: pread failed: " +
                            std::string(std::strerror(err)));
                    }
                    if (got == 0) {
                        throw std::runtime_error(
                            "DPMoERuntime::move_chunk: short pread for " + wid);
                    }
                    total += got;
                }
                if (fd == gguf_fd_) {
            ...
            if (host.any_precision) {
                if (p < 0 || p >= (int)host.chunk_planes.size()) {
                    throw std::runtime_error(
                        "DPMoERuntime::move_chunk: any-prec chunk index "
                        + std::to_string(p) + " out of range for " + wid);
                }
                expected_disk_bytes = host.chunk_planes[p].disk_chunk_bytes;
                kernel_chunk_bytes  = host.chunk_planes[p].kernel_chunk_bytes;
            ...
            if ((size_t)size != expected_source_bytes) {
                throw std::runtime_error(
                    "DPMoERuntime::move_chunk: SSD-stream plane size "
                    "doesn't match expected disk layout for " + wid +
            ...
            }
            {
                ...
                if (kernel_ready_source ||
                    ...
                    throw std::runtime_error(
                        "DPMoERuntime::move_chunk: missing "
        ...
        throw std::runtime_error(
            "DPMoERuntime::move_chunk: unknown cid " +
            std::to_string(cid));
    }
    ...
    while (h.device_ptr == nullptr) {
        profile_record_pool_make_room();
        {
            ...
            if (!scheduler_->make_room_for(*pool_, nbytes)) {
                throw std::runtime_error(
                    "DPMoERuntime::move_chunk: pool full and scheduler "
    ...
    if (src_ptr_is_ephemeral && h.ready_event != nullptr) {
        ...
        if (e_sync != cudaSuccess) {
            throw std::runtime_error(
                "DPMoERuntime::move_chunk: H2D wait failed for " + wid +
...
bool DPMoERuntime::submit_async_load(const std::string & wid, int cid) {
...
bool DPMoERuntime::submit_async_load(
...
void DPMoERuntime::wait_async_load_idle() {
...
void DPMoERuntime::wait_async_load_batch(
...
void DPMoERuntime::batch_pread_worker_loop_() {
    for (;;) {
...
void DPMoERuntime::io_worker_loop_() {
    for (;;) {
...
void DPMoERuntime::set_replay_kbar(float kbar) {
    std::lock_guard<std::mutex> lk(replay_dial_mu_);
    replay_kbar_ = kbar;
}

float DPMoERuntime::current_replay_kbar() const {
    std::lock_guard<std::mutex> lk(replay_dial_mu_);
    return replay_kbar_;
}

void DPMoERuntime::set_replay_dial_version(uint64_t v) {
    replay_dial_version_.store(v, std::memory_order_release);
}

uint64_t DPMoERuntime::replay_dial_version() const {
