// DPMoE — runtime singleton. Owns a VRAM pool + a Scheduler.
// Core primitive:
//
...
namespace dp_moe_ext { struct NaverKernelScratch; }
...
class DPMoERuntime {
public:
    DPMoERuntime(size_t capacity_bytes, int device, bool copy_stream,
                     const char * scheduler_name);
    ~DPMoERuntime();

    DPMoERuntime(const DPMoERuntime &) = delete;
    DPMoERuntime & operator=(const DPMoERuntime &) = delete;
    ...
    // stay under DP_MOE_HOST_CAP_MB. Acquires the per-Entry host
...
public:
    ...
    // capture when the dial changes (e.g. HTTP /dp_moe/kbar
