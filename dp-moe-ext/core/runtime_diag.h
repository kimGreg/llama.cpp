// DPMoE — cache-stack diagnostics.
//
// Per-rank / per-plane tier-hit buckets + DP_MOE_TRACE_FILE
// chunk-request recorder. Pure measurement infrastructure: not load-
// bearing for runtime correctness or hot-path TPS.
//
// Gated by the DP_MOE_DIAG cmake option. When OFF (default), the
...
class DPMoERuntime;     // forward decl
...
// AND, when DP_MOE_GATE_TRACE_FILE is set, appends a CSV row
...
inline void record_chunk(const DPMoERuntime &,
                         const std::string &,
                         int, int, int) {}
