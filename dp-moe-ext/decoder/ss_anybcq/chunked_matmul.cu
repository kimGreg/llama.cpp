// DPMoE / decoder / ss_anybcq — chunked-matmul impl.
//
...
using ::dp_moe_ext::naver_gemv_launch;
using ::dp_moe_ext::naver_gemm_launch;
using ::dp_moe_ext::launch_dequant_planes_f16;
using ::dp_moe_ext::batched_gemm_f16;
    ...
    StreamHandle compute_stream)
{
    ...
    // chunked LUT-GEMM under DP_MOE_BATCHED_BACKEND=fused. See
    // commit history for the rationale (cuBLAS wins at N=2048 in the
    // current tuning regime).
    const char * backend = std::getenv("DP_MOE_BATCHED_BACKEND");
...
// Used to live as ``DPMoERuntime::chunk_matmul[_batched]`` on core,
