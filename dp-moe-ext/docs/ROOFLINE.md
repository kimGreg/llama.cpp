# Roofline analysis — DPMoE on RTX 4070 Ti SUPER + NVMe

Hardware bandwidths and FLOPS measured on this box, applied against
the dp_moe runtime's measured per-token byte traffic + cache hit
...
DPMoE streams **only the FFN routed-expert weights**. Attention,
...
Active policy is **`DP_MOE_MOE_POLICY=score`**: per-(t, u) gate
...
France is"`, `DP_MOE_VRAM_CAP_MB=8192`, no host cap, 8 prefetch
...
```
29.50%  dp_moe_ext::plane_disk_to_kernel
...
1. **Right-size the host LRU.** The DRAM cache is providing 5 %
   ...
   `DP_MOE_HOST_CAP_MB` triggers an LRU eviction. A sweep over
   `DP_MOE_HOST_CAP_MB ∈ {8, 16, 32}` GB at the saturated
