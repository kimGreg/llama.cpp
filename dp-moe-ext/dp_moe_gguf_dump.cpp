// dp_moe-gguf-dump — read a GGUF and print its dp_moe.* metadata.
//
// Usage:
//   dp_moe-gguf-dump path/to/qwen3-1.7b-anybcq-P8.gguf
//
// Exits 0 if dp_moe metadata is present and byte accounting checks
// pass; 2 if the file has no dp_moe metadata (i.e. it's a stock
...
int main(int argc, char ** argv) {
    ...
    int rc = 0;
    try {
        auto reader = dp_moe_ext::StreamReader::from_gguf(ctx, path);
        if (!reader.has_value()) {
            std::fprintf(stderr,
                "no dp_moe metadata in %s (stock GGUF?)\n", path);
            rc = 2;
        } else {
            dp_moe_ext::dump(*reader);
        ...
        std::fprintf(stderr, "DPMoE error: %s\n", e.what());
        rc = 1;
    }
