...
// MoE chunk-movement profiling counters. Enabled by DP_MOE_PROFILE=1
...
// Public counter accessors for /dp_moe/stats. Each is a single
...
bool size_scratch_for(const StreamReader & r) {
    ...
    g_batch_n_max = 2048;
    if (const char * s = getenv("DP_MOE_BATCH_N_MAX")) {
...
void print_profile_if_enabled() {
    if (!getenv("DP_MOE_PROFILE")) return;
...
// dp_moe_try_cuda_mul_mat). The previous dense streaming body
...
// ``dp_moe_pre_op`` → ``forward_moe_layer``).
