// DPMoE / qwen3 — fused MoE LUT-GEMV impl.
//
...
using ::dp_moe_ext::MoeExpertTable;
    ...
    StreamHandle stream)
{
    ...
    dp_moe_ext::launch_diag::note_launch(
        dp_moe_ext::launch_diag::Kind::QbiasRefresh);
    ...
    StreamHandle stream)
{
    ...
    if (!logged) {
        std::fprintf(stderr,
            "DPMoE: refresh_alpha_for_anyprec_launch active "
    ...
    auto s = (cudaStream_t) stream;
    dp_moe_ext::launch_diag::note_launch(
        dp_moe_ext::launch_diag::Kind::AlphaBetaRefresh);
    ...
    Activation    act)
{
    ...
    dp_moe_ext::launch_diag::note_launch(
        dp_moe_ext::launch_diag::Kind::SwigluMul);
    switch (act) {
    ...
    StreamHandle  stream)
{
    ...
    dp_moe_ext::launch_diag::note_launch(
        dp_moe_ext::launch_diag::Kind::WeightedReduce);
    ...
    if (tid == 0) {
        ...
        for (int e = 0; e < n_expert; ++e) {
            ...
            if (K_i < K_min || K_i > K_max) {
                printf("dp_moe-allocator[gpu]: K[%d]=%d outside "
                        "[%d, %d] at layer (kbar=%g)\n",
