// DPMoE / qwen3 — Qwen3 MoE AnyBCQ executor.
//
...
// by ``dp_moe.executor = "qwen3_ss_anybcq_v1"`` in the GGUF.
//
// ``forward_moe_layer`` is invoked once per managed MoE layer by the
// per-layer sentinel rail (qwen3_runtime_glue's ``dp_moe_pre_op``
// matches on the ``"dp_moe.moe_layer_<L>"`` name set by the
...
public:
    ...
    void bind_to_model(DPMoERuntime &  rt,
    ...
    // the extern-C ``dp_moe_stat_required_set_misses`` accessor.
