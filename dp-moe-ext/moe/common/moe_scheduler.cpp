...
// the file.
void register_chunk_io_from_layout(DPMoERuntime & rt,
...
// residual table from DP_MOE_KBAR_PROFILE_FILE; uniform mode rounds
...
public:
    ...
    void on_install(DPMoERuntime & rt,
                    const StreamReader & reader,
        ...
        // DPMoERuntime::managed_names_ but moved here in P2★
        ...
        size_t n_total = 0;
        // DRAM cache cap (host tier). DP_MOE_HOST_CACHE=0 disables
        ...
        if (host_cache_enabled_ &&
            (std::getenv("DP_MOE_HOST_CAP_MB") != nullptr)) {
            const char * s = std::getenv("DP_MOE_HOST_CAP_MB");
            long mb = std::atol(s);
        ...
        for (const auto & name : reader.chunked_tensor_names()) {
            ...
            //
            // Override: DP_MOE_KEEP_HOST_BYTES=1 keeps every chunk
            ...
            const char * keep_env = std::getenv("DP_MOE_KEEP_HOST_BYTES");
        ...
        std::fprintf(stderr,
            "dp_moe-scheduler[moe]: %zu experts | max_chunks=%d",
        ...
        if (const char * path = std::getenv("DP_MOE_KBAR_PROFILE_FILE")) {
            std::FILE * f = std::fopen(path, "rb");
            if (f == nullptr) {
                std::fprintf(stderr,
                    "dp_moe-scheduler[moe]: DP_MOE_KBAR_PROFILE_FILE=%s "
                    "fopen failed: %s\n", path, std::strerror(errno));
            } else {
                uint32_t hdr[4] = {0, 0, 0, 0};
                if (std::fread(hdr, sizeof(uint32_t), 4, f) != 4) {
                    std::fprintf(stderr,
                        "dp_moe-scheduler[moe]: residuals file header short\n");
                    std::fclose(f);
                } else {
                    ...
                    if (n_lay > 0 && n_exp > 0 && Kmn >= 0 && nK > 0 &&
                        ...
                        if (std::fread(R.data(), sizeof(float), n, f) == n) {
                            ...
                            std::fprintf(stderr,
                                "dp_moe-scheduler[moe]: short read on residuals "
                                "(want %zu floats)\n", n);
                        }
                    } else {
                        std::fprintf(stderr,
                            "dp_moe-scheduler[moe]: bad residuals header "
                            "L=%d E=%d K=[%d, %d]\n", n_lay, n_exp, Kmn, Kmx);
                    }
                    std::fclose(f);
                }
        ...
        if (const char * mode = std::getenv("DP_MOE_KBAR_ALLOCATOR")) {
        ...
        std::fprintf(stderr,
            "dp_moe-scheduler[moe]: built %zu expert tables eagerly\n",
            canonical_seen.size());

        // Build one MoEMatMulComp per managed canonical.  The dispatch
        // shim hands these off to ``DPMoERuntime::run`` (Step 2 of
        // the unified chunked-load + compute lifecycle).  We size each
        // comp's pinned input buffers off DP_MOE_BATCH_N_MAX
        // (default 2048; same env knob that sizes the MoE scratch).
        int max_n_tokens = 2048;
        if (const char * s = std::getenv("DP_MOE_BATCH_N_MAX")) {
  ...
  public:
    ...
    const MoeExpertTable * moe_expert_table(
        ...
        for (int e = 0; ; ++e) {
            ...
            if (d_qw == nullptr || d_alpha == nullptr) {
                std::fprintf(stderr,
                    "dp_moe-scheduler[moe]: per-plane device pointer "
        ...
        if (t.d_qw_planes_per_expert == nullptr ||
            ...
            std::fprintf(stderr,
                "dp_moe-scheduler[moe]: small_alloc exhausted for %s\n",
                canonical_wid.c_str());
            return nullptr;
        }
        ...
        if (any_any_prec) {
            ...
            if (t.d_qbias_slot_per_expert == nullptr) {
                std::fprintf(stderr,
                    "dp_moe-scheduler[moe]: small_alloc exhausted for "
    ...
    bool claims_node(const struct ggml_tensor * node) const override {
        // dp_moe always dispatches managed mul_mat / mul_mat_id
    ...
    bool claims_tensor(const struct ggml_tensor * w) const override {
        ...
        // ``dp_moe_try_cuda_mul_mat`` boundary so the abort
    ...
    //
    // Optional diagnostic: set DP_MOE_DEBUG_GRAPH_WALK=1 to log
    ...
    void on_graph_compute_begin(StreamHandle compute_stream,
        ...
        // S8 (Mode A): cgraph audit. DP_MOE_CGRAPH_AUDIT=1 walks
        // the cgraph and proves the two post-cutover invariants:
        //   (a) every node named ``"dp_moe.moe_layer_<N>"`` is an
        ...
        if (const char * a = getenv("DP_MOE_CGRAPH_AUDIT")) {
            if (a[0] && a[0] != '0') {
                ...
                for (int i = 0; i < n_nodes; ++i) {
                    ...
                    if (is_sentinel) {
                        ...
                        if (node->src[0] == nullptr ||
                            ...
                            std::fprintf(stderr,
                                "[dp_moe-audit] sentinel '%s' missing "
                                "src[0..3]: [%p, %p, %p, %p]\n",
                                node->name,
                    ...
                    if (node->op == GGML_OP_MUL_MAT_ID) {
                        ...
                        if (w && w->name[0] && managed_names_.count(w->name)) {
                            ++n_managed_mmid;
                            std::fprintf(stderr,
                                "[dp_moe-audit] managed MUL_MAT_ID "
                    ...
                    if (node->op == GGML_OP_MUL_MAT) {
                        ...
                        if (w && w->name[0] && managed_names_.count(w->name)) {
                            ++n_managed_mm;
                            std::fprintf(stderr,
                                "[dp_moe-audit] managed MUL_MAT (dense) "
                                "leaked into cgraph: src0='%s' node='%s'\n",
                                w->name, node->name);
                        }
                    }
                ...
                std::fprintf(stderr,
                    "[dp_moe-audit] nodes=%d sentinels=%d "
    ...
    bool set_dynamic_residuals(const std::vector<float> & R_flat,
        ...
        std::fprintf(stderr,
            "dp_moe-scheduler[moe]: dynamic_residuals SET — n_layers=%d "
            "n_experts=%d K=[%d, %d]\n",
    ...
    void upload_dynamic_R_to_device_unlocked() {
        ...
        if (dynamic_R_d_ == nullptr) {
            ...
            if (err != cudaSuccess) {
                std::fprintf(stderr,
                    "dp_moe-scheduler[moe]: cudaMalloc(dynamic_R_d, "
    ...
    bool set_prefill_decay_end(uint64_t n_tokens, bool reset_state) {
        if (n_tokens == 0) return false;
        schedule_pp_end_token_ = n_tokens;
        phase_schedule_enabled_ = true;
        if (reset_state) {
            schedule_prefill_seen_tokens_ = 0;
            schedule_tg_token_ = 0;
            schedule_in_decode_ = false;
            schedule_last_kbar_ = dynamic_kbar_.load(std::memory_order_relaxed);
        }
        dial_version_.fetch_add(1, std::memory_order_relaxed);
        return true;
    }
    ...
    bool allocate_dispatch_budget(int layer,
        ...
        if (K_min < 0 || K_max < K_min) {
            GGML_ABORT("dp_moe-allocator: invalid K range [%d, %d]",
                       K_min, K_max);
        }
        if (K_max > 255) {
            GGML_ABORT("dp_moe-allocator: K_max=%d exceeds uint8_t trace capacity",
                       K_max);
        }
        const int n_K   = K_max - K_min + 1;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (K_i < K_min || K_i > K_max) {
                std::fprintf(stderr,
                    "dp_moe-allocator: invariant 2 violated — "
                    "K[%d]=%d outside [%d, %d]\n",
                    i, K_i, K_min, K_max);
                GGML_ABORT("dp_moe-allocator: per-expert K out of range");
            }
            check_sum += K_i;
        }
        ...
        if (check_sum != (long) target_b) {
            std::fprintf(stderr,
                "dp_moe-allocator: invariant 1 violated — "
                "sum K[e]=%ld, expected target_b=%d "
                "(mode=kbar, KBar=%g n=%d)\n",
                check_sum, target_b, (double) kbar, n);
            GGML_ABORT("dp_moe-allocator: budget not enforced");
        }
        ...
        if (log_env && log_env[0] && log_env[0] != '0' &&
            ...
            std::fprintf(stderr,
                "dp_moe-alloc L=%d n=%d mode=kbar  KBar_eff=%.3f  B=%d  "
...
private:
    ...
    DPMoERuntime * rt_ = nullptr;

    void init_phase_schedule_() {
        const float base = dynamic_kbar_.load(std::memory_order_relaxed);
        bool any = false;
        float v = 0.0f;
        if (read_float_env_opt("DP_MOE_KBAR_PP_BEGIN", v)) {
            schedule_pp_begin_ = v; any = true;
        } else {
            schedule_pp_begin_ = base;
        }
        if (read_float_env_opt("DP_MOE_KBAR_PP_END", v)) {
            schedule_pp_end_ = v; any = true;
        } else {
            schedule_pp_end_ = schedule_pp_begin_;
        }
        if (read_float_env_opt("DP_MOE_KBAR_TG_BEGIN", v)) {
            schedule_tg_begin_ = v; any = true;
        } else {
            schedule_tg_begin_ = base;
        }
        if (read_float_env_opt("DP_MOE_KBAR_TG_END", v)) {
            schedule_tg_end_ = v; any = true;
        } else {
            schedule_tg_end_ = schedule_tg_begin_;
        }
        schedule_tg_decay_start_ =
            read_u64_env("DP_MOE_KBAR_TG_DECAY_START", 0);
        schedule_tg_decay_end_ =
            read_u64_env("DP_MOE_KBAR_TG_DECAY_END",
                         schedule_tg_decay_start_);
        schedule_pp_end_token_ =
            read_u64_env("DP_MOE_KBAR_PP_DECAY_END", 0);
        if (std::getenv("DP_MOE_KBAR_TG_DECAY_START") ||
            std::getenv("DP_MOE_KBAR_TG_DECAY_END") ||
            std::getenv("DP_MOE_KBAR_PP_DECAY_END")) {
            any = true;
        }
        ...
        if (phase_schedule_enabled_) {
            std::fprintf(stderr,
                "dp_moe-scheduler[moe]: KBar schedule PP %.3f->%.3f "
    ...
    // Profile residuals loaded from DP_MOE_KBAR_PROFILE_FILE.
    ...
    bool     logical_cache_age_ =
        read_int_env("DP_MOE_MOE_LOGICAL_CACHE_AGE", 0) != 0;
    ...
    MoEResidencyTracker tracker_{
        read_float_env("DP_MOE_MOE_EVICT_PLANE_WEIGHT", 8.0f),
        read_float_env("DP_MOE_MOE_EVICT_AGE_WEIGHT",   1.0f),
        read_float_env("DP_MOE_MOE_EVICT_FREQ_WEIGHT",  0.0f),
        read_i64_env("DP_MOE_MOE_EVICT_AGE_PLANE_TIE_WINDOW", -1),
    };
    ...
    // surface that used to live on DPMoERuntime.
    ...
    // hand off to ``DPMoERuntime::run``.  Owned by the scheduler;
    ...
    // ``"dp_moe.moe_layer_<L>"`` name; the cgraph audit
    // (DP_MOE_CGRAPH_AUDIT=1) is the only walk that still touches
    // every node.
};
...
std::unique_ptr<Scheduler> make_scheduler(const char * which) {
    ...
    std::fprintf(stderr,
        "dp_moe-scheduler: unknown scheduler '%s'; "
...
bool scheduler_set_prefill_decay_end(
    ...
    return as_moe(sched).set_prefill_decay_end(n_tokens, reset_state);
}
