// DPMoE — end-to-end GPU kernel test.
//
// Reads a dp_moe GGUF, picks one managed tensor, builds the
...
//
// Usage:
//   dp_moe-gpu-test <gguf> <tensor> <input_fp16.bin> <expected_y_fp16.bin>
//
// The input + expected_y are produced by
// dp_moe/scripts/dump_gpu_reference.py in the parent repo.
...
using dp_moe_ext::StreamReader;
using dp_moe_ext::TensorLayout;
using dp_moe_ext::VramChunkPool;
using dp_moe_ext::build_upstream_layout_host;
...
int main(int argc, char ** argv) {
    ...
    if (!reader) { std::fprintf(stderr, "no dp_moe metadata\n"); return 1; }
