...
namespace streamllm_ext { namespace diag {
...
// Optional per-event CSV trace. Open at install via STREAMLLM_GATE_TRACE_FILE.
...
// install when STREAMLLM_TRACE_FILE is set; closed at clear. Format
...
void install_open_trace() {
    if (const char * tf = std::getenv("STREAMLLM_TRACE_FILE")) {
        if (tf[0] && g_trace_fp == nullptr) {
            ...
            if (fp) {
                ...
                g_trace_fp = fp;
                std::fprintf(stderr,
                    "streamllm-ext: chunk trace recording → %s\n", tf);
            } else {
                std::fprintf(stderr,
                    "streamllm-ext: failed to open trace file %s\n", tf);
            }
        }
    }
    if (const char * gtf = std::getenv("STREAMLLM_GATE_TRACE_FILE")) {
        if (gtf[0] && g_gate_trace_fp == nullptr) {
            ...
            if (fp) {
                ...
                g_gate_trace_fp = fp;
                std::fprintf(stderr,
                    "streamllm-ext: gate-event trace recording → %s\n", gtf);
            } else {
                std::fprintf(stderr,
                    "streamllm-ext: failed to open gate trace file %s\n", gtf);
            }
        }
    }
}
...
void clear_close_trace_and_dump() {
    ...
    // The dump only fires when the user set STREAMLLM_STATS or
    // STREAMLLM_PROFILE. (The same env conventions used by the rest
    // of the runtime's diagnostic output.) Saves spamming stderr on
    // ordinary runs even when DIAG was compiled in.
    const bool want_stats   = std::getenv("STREAMLLM_STATS")   != nullptr;
    const bool want_profile = std::getenv("STREAMLLM_PROFILE") != nullptr;
    ...
    if (want_stats) {
        ...
        if (tot_att > 0) {
            ...
            std::fprintf(stderr,
                "streamllm-ext tier hits: attempts=%lu  VRAM=%.1f%% (%lu)  "
            ...
            // (STREAMLLM_TRACE_FILE) + experiments/analyze_expert_chunks.py.
            ...
            std::fprintf(stderr,
                "streamllm-ext chunk pattern: requests=%lu  ssd_loaded=%lu "
    ...
    // gate trace CSV (when STREAMLLM_GATE_TRACE_FILE is set) for
...
int streamllm_stat_diag_enabled(void) { return 1; }
} // extern "C"
