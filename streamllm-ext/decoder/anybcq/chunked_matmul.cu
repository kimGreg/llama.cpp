// streamllm-ext / decoder / anybcq — MoE chunked-matmul dispatch impl.
//
    ...
    StreamHandle             stream)
{
    ...
    if (prec_per_eid_d == nullptr || host_n_chunks_per_eid == nullptr ||
        ids_d == nullptr || X_fp16 == nullptr || Y_slot_f32 == nullptr ||
        n_experts <= 0 || n_tokens <= 0 || n_used <= 0)
    {
        std::fprintf(stderr,
            "streamllm-ext: anybcq::moe_chunk_matmul(%s): bad inputs "
    ...
    if (n_chunks_max <= 0) {
        std::fprintf(stderr,
            "streamllm-ext: anybcq::moe_chunk_matmul(%s): empty layout "
    ...
    for (int e = 0; e < n_experts; ++e) {
        ...
        if (planes < 0) {
            // Out of range: clamp to a safe value and count.  plan()
            // shouldn't have produced this; logging once per call
            // suffices (the counter accumulates everything).
            g_n_chunks_oob.fetch_add(1, std::memory_order_relaxed);
#ifndef NDEBUG
            std::fprintf(stderr,
                "streamllm-ext: anybcq::moe_chunk_matmul(%s): "
    ...
    //    non-null at launch (SSOT §6.9 M1).
    {
        ...
        if (err != cudaSuccess) {
            std::fprintf(stderr,
                "streamllm-ext: anybcq::moe_chunk_matmul(%s): H2D "
